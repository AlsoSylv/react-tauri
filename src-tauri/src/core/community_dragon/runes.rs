...
impl CommunityDragon {
    pub async fn runes(&self) -> Result<Vec<Runes>, CommunityDragonError> {
        ...
        let request = request::<Vec<Runes>, CommunityDragonError>(url, &self.client, CommunityDragonError::CommunityDragonMissing, CommunityDragonError::CannotConnect).await;
        match request {
            Ok(valid) => Ok(valid),
            Err(err) => Err(err),
        }
    ...
    pub async fn runes_style(&self) -> Result<RunesStyle, CommunityDragonError> {
        ...
        let request = request::<RunesStyle, CommunityDragonError>(url, &self.client, CommunityDragonError::CommunityDragonMissing, CommunityDragonError::CannotConnect).await;
        match request {
            Ok(valid) => Ok(valid),
            Err(err) => Err(err),
        }
